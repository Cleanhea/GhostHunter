...
| 핵심 메커닉 | 레이캐스트 타겟팅 → 홀드 → 1인 밀치기 / 2인 고정 운반(휠 회전·기울이기) → 발사 → 연쇄 충돌 |
...
- **청소·이사 작업 시스템** — 대걸레 장착·좌클릭 얼룩 제거·B안 랜덤 얼룩·F1 초기화 프로토타입을 추가했다(2026-09-12). 실제 얼룩의 Q 탐지 마커는 연동한다. 정식 얼룩 규칙·가구 완료·진행도·귀신 40% 연동은 미정 유지 → [cleaning-system.md](cleaning-system.md)
